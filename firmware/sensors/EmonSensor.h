...
class EmonSensor : public I2CSensor {
    ...
    public:
        ...
        void expectedPower(uint8_t channel, unsigned int expected) {
        ...
        void resetEnergy(uint8_t channel, double value = 0) {
            if (channel >= _channels) return;
        ...
        void setCurrentRatio(uint8_t channel, double current_ratio) {
            if (channel >= _channels) return;
        ...
        double getCurrentRatio(uint8_t channel) {
    ...
    protected:
        ...
        virtual unsigned int readADC(uint8_t channel) = 0;

        void calculateFactors(uint8_t channel) {
        ...
        uint8_t _channels = 0;                    // Number of ADC channels available
        uint8_t _magnitudes = 0;                  // Number of magnitudes per channel
        uint32_t _samples = EMON_MAX_SAMPLES;      // Samples (dynamically modificable)

        uint8_t _resolution = 10;                 // ADC resolution in bits
        uint32_t _adc_counts;                      // Max count

...
double value(uint8_t index) {
        uint8_t i = 0;
            #if GEIGER_REPORT_CPM
        if (index == i++) {
                uint32_t _period_begin = _lastreport_cpm;
            ...
            #if GEIGER_REPORT_SIEVERTS
        if (index == i++) {
                uint32_t _period_begin = _lastreport_sv;
...
void handleInterrupt(uint8_t gpio) {
        UNUSED(gpio);
...
void _attach(GeigerSensor * instance, uint8_t gpio, uint8_t mode);
void _detach(uint8_t gpio);

void _enableInterrupts(bool value) {

        static uint8_t _interrupt_gpio = GPIO_NONE;
...
volatile uint32_t _events = 0;
volatile uint32_t _ticks = 0;

uint32_t _debounce = GEIGER_DEBOUNCE;
...
uint32_t _lastreport_cpm = millis();
uint32_t _lastreport_sv = _lastreport_cpm;
...
void ICACHE_RAM_ATTR _geiger_sensor_isr(uint8_t gpio) {
...
void GeigerSensor::_attach(GeigerSensor * instance, uint8_t gpio, uint8_t mode) {
    ...
    #if SENSOR_DEBUG
        DEBUG_MSG(PSTR("[GEIGER] GPIO%d interrupt attached to %s\n"), gpio, instance->description().c_str());
    #endif
}

void GeigerSensor::_detach(uint8_t gpio) {
        if (!gpioValid(gpio)) return;
        uint8_t index = gpio > 5 ? gpio-6 : gpio;
        if (_geiger_sensor_instance[index]) {
                detachInterrupt(gpio);
        #if SENSOR_DEBUG
                DEBUG_MSG(PSTR("[GEIGER] GPIO%d interrupt detached from %s\n"), gpio, _geiger_sensor_instance[index]->description().c_str());
        #endif

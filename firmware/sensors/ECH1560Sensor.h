...
class ECH1560Sensor : public BaseSensor {
    ...
    protected:
        ...
        void _attach(ECH1560Sensor * instance, uint8_t gpio, uint8_t mode);
        void _detach(uint8_t gpio);

        void _enableInterrupts(bool value) {

            static uint8_t _interrupt_clk = GPIO_NONE;
        ...
        void _sync() {
            ...
            if (byte2 == 0) {
                _dosync = false;
            #if SENSOR_DEBUG
                DEBUG_MSG(PSTR("Nothing connected, or out of sync!\n"));
            #endif
            }
        }
...
void ICACHE_RAM_ATTR _ech1560_sensor_isr(uint8_t gpio) {
...
void ECH1560Sensor::_attach(ECH1560Sensor * instance, uint8_t gpio, uint8_t mode) {
...
void ECH1560Sensor::_detach(uint8_t gpio) {
    if (!gpioValid(gpio)) return;
    uint8_t index = gpio > 5 ? gpio-6 : gpio;
    if (_ech1560_sensor_instance[index]) {

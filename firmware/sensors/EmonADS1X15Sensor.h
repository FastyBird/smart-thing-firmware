...
class EmonADS1X15Sensor : public EmonSensor {
    ...
    public:
        ...
        String slot(uint8_t index) {
            char buffer[35];
            uint8_t channel = getChannel(index % _ports);
        ...
        uint8_t type(uint8_t index) {
            uint8_t magnitude = index / _ports;
            uint8_t i = 0;
        ...
        void pre() {
            ...
            for (uint8_t port = 0; port < _ports; port++) {
                uint8_t channel = getChannel(port);
    ...
    protected:
        ...
        uint8_t getChannel(uint8_t port) {
            uint8_t count = 0;
            uint8_t bit = 1;
            for (uint8_t channel = 0; channel < ADS1X15_CHANNELS; channel++) {
                if ((_mask & bit) == bit) {
        ...
        void setConfigRegistry(uint8_t channel, bool continuous, bool start) {

...
class BMX280Sensor : public I2CSensor {
	...
	signed char decimals(uint8_t type) { 
        ...
        static void manifest(JsonArray& sensors) {
...

            {
                ...
                for (uint8_t i = 0; i < sizeof(BMX280Sensor::addresses); i++) {
                    JsonObject& option = options.createNestedObject();
...
uint8_t BMX280Sensor::addresses[2] = {0x76, 0x77};

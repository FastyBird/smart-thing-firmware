...
class CSE7766Sensor : public BaseSensor {
    ...
    protected:
        ...
        void _process() {
            ...
            uint32_t _coefV = (_data[2]  << 16 | _data[3]  << 8 | _data[4] );              // 190770
            uint32_t _coefC = (_data[8]  << 16 | _data[9]  << 8 | _data[10]);              // 16030
            uint32_t _coefP = (_data[14] << 16 | _data[15] << 8 | _data[16]);              // 5195000
            ...
            _voltage = 0;
            if ((adj & 0x40) == 0x40) {
                uint32_t voltage_cycle = _data[5] << 16 | _data[6] << 8 | _data[7];        // 817
            ...
            if ((adj & 0x10) == 0x10) {
                if ((_data[0] & 0xF2) != 0xF2) {
                    uint32_t power_cycle = _data[17] << 16 | _data[18] << 8 | _data[19];   // 4709
            ...
            if ((adj & 0x20) == 0x20) {
                if (_active > 0) {
                    uint32_t current_cycle = _data[11] << 16 | _data[12] << 8 | _data[13]; // 3376
